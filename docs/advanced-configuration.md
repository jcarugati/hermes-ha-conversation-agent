...
- Si queda en blanco, cada solicitud usa el modelo predeterminado anunciado por `/v1/capabilities` de la API Hermes que está en ejecución.
- Si se completa, el alias debe existir ya en las rutas de modelos configuradas de esa API Hermes. La integración envía ese alias como el único valor de `model`.
...
El campo admite como máximo 512 caracteres. Si no estás seguro de que una ruta exista, déjalo vacío: así conservas el modelo predeterminado del servidor.

## Endpoint directo y contrato

La URL configurada es la raíz de la API, no una URL de chat ni de una pasarela. La integración realiza estas comprobaciones durante la configuración, al cargar la entrada y antes de cada solicitud:
...
3. La solicitud se envía como `POST /v1/responses` autenticado, sin redirecciones y con `stream: false`.
...
Para cada turno de Assist, la integración genera una clave de conversación opaca y nueva. No lee ni envía el `ChatLog` entrante de Home Assistant. Por diseño, no hay un control para restablecer, escoger, recuperar o reutilizar historial remoto: no existe historial conversacional de Assist que la integración conserve o reenvíe entre turnos.
...
Mantén el servidor en una red privada y sin CORS de navegador innecesario. HTTPS verifica el certificado por defecto. HTTP sin cifrar solo puede habilitarse de forma explícita para hosts locales o privados y expone el token y los datos de la solicitud a la red.
...
Mantén la ruta de voz que ya funciona mientras haces la primera comprobación: primero texto de Assist, luego una consulta de solo lectura y por último una acción inocua autorizada. Consulta también la [política de seguridad](../SECURITY.md).
